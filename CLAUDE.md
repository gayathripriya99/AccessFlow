...
Progress is tracked per-phase in `backend/Phase-XX.md` files and in the root [README.md](README.md) phase table. **Phases 1 (Authentication), 2 (Users/Roles/Permissions CRUD), 3 (Authorization Middleware), 4 (Protected UI — React), 5 (Audit Logs), 6 (Permission Simulator), 7 (Advanced RBAC — role hierarchy), and 8 (ABAC) are complete.** Phase 8 was originally "ABAC + Multi-tenant" — scoped down to ABAC only by explicit user decision; **multi-tenant remains unstarted** (see `backend/Phase-08.md`). Read the most recent `Phase-XX.md` before starting new work to know exactly what exists. The repo is pushed to `github.com/gayathripriya99/AccessFlow` (`main` branch) via a dedicated repo-local git identity/SSH key — see `chatdocument_doc_file/` for the exact setup history if that ever needs revisiting.
...
`frontend-react/` layering: `src/api/*` (axios client + one file per resource — `auth`/`users`/`roles`/`permissions`/`auditLogs`/`simulator` — plus shared `types.ts`/`errors.ts`), `src/auth/*` (`AuthContext` — in-memory access token + silent-refresh-on-mount, tracks `sessionExpired` separately from a plain logged-out state, `ProtectedRoute` — redirects unauthenticated to `/login`, `PermissionGate` — hides inline elements, `RequirePermission` — route-level guard rendering `Forbidden`), `src/pages/*` (one per route: `LoginPage`, `RegisterPage`, `DashboardPage`, `UsersPage`, `RolesPage`, `PermissionsPage`, `AuditLogsPage`, `PermissionSimulatorPage`, `PoliciesPage`, `NotFoundPage`), `src/components/*` (generic, reused across pages — `DataTable`, `Pagination`, `SearchInput`, `FormField`, `Button`, `Layout`, `Navbar`, `Sidebar`, `Forbidden`, `ErrorState`, `LanguageSwitcher`), `src/config/navigation.ts` (single `NAV_ITEMS` source Sidebar + Navbar's mobile drawer both render from, permission-filtered), `src/i18n/*` (4 locales: en/hi/kn/fr). See `backend/Phase-04.md` through `backend/Phase-08.md` for the full rundown.
...
- `src/constants/systemPermissions.ts` — the 17 baseline permissions + `admin` role name used by first-user bootstrap (see below). `src/constants/systemPolicies.ts` — the 3 baseline ABAC policies, seeded the same way.
...
- **Never** check `role === "Admin"` directly anywhere in authorization logic — always resolve and evaluate the permission set, frontend included (the React app gates on `permissions: string[]` from `/auth/me`, never on role name). `AuthorizationService.resolveAccess(userId)` is the one place this resolution happens (populates `roles` → `roles.permissions`, then — since Phase 7 — also walks each role's ancestor chain via `RoleRepository.getAncestorChain` so inherited permissions count too, and returns both role names and the flattened permission-name set); `getPermissionNames(userId)` is a thin wrapper around it kept for `requirePermission(name)`. `AuthService.getCurrentUser` (backing `GET /auth/me`) also calls `resolveAccess` directly, since Phase 4 — the frontend needs both role names (for display) and permissions (for gating), not just identity.
- **Role hierarchy (Phase 7):** a `Role` may have a `parentRole` (self-referencing, nullable). A role's *effective* permissions are its own unioned with every ancestor's, transitively — resolved on every permission check (`AuthorizationService`) and by the Permission Simulator (`SimulatorService`), not just reported by the Role API. Cycles (self-parenting or a transitive loop) are rejected with 400 at create/update time (`RoleRepository.wouldCreateCycle`); deleting a role nulls out `parentRole` on any children rather than leaving a dangling reference. See `backend/Phase-07.md`.
...
- **Admin bootstrap:** the very first user ever registered (`UserRepository.count() === 1` at that moment) automatically gets an `admin` role with all 17 baseline permissions, via `AdminBootstrapService.bootstrapFirstUserAsAdmin`, called from `AuthService.register` — which since Phase 8 also seeds the 3 baseline ABAC policies (`SYSTEM_POLICIES`) the same idempotent way. Every later registration gets zero roles — access must be granted explicitly via `PATCH /users/:id`. This also means the first `createAuthenticatedUser()` call in any test file becomes that test's admin (each test's in-memory DB is cleared beforehand); tests needing a guaranteed non-admin user should use `createNonAdminUser` from `tests/helpers/authenticatedUser.ts` instead.
- **ABAC (Phase 8):** a `Policy` (resource+action+effect+conditions) sits alongside RBAC, evaluated by `AbacService` and enforced via `requireAccess` (used only where a route has a single `:id` to evaluate against — list endpoints stay RBAC-only). An explicit `deny` policy always wins, even over a real RBAC grant; an `allow` policy grants access even without the matching permission; no match falls back to an ordinary `requirePermission` check. The 3 seeded policies (`self-service-read`/`self-service-update`/`deny-self-delete`) all compare `resource.id == subject.id` — the only meaningful "attribute" in this domain, since Users are the only resource anyone could plausibly be said to "own" (their own account). **`UserService.update` enforces that a self-service update granted only via ABAC (not real `users.update`) may change `name` only** — without this, "update your own profile" would double as "grant yourself any role." See `backend/Phase-08.md`.
- Deleting a `Permission`/`Role` cascades: it's pulled out of every `Role`/`User` that referenced it, and (since Phase 7) any child roles have their `parentRole` nulled out too (see `PermissionRepository.deleteById`/`RoleRepository.deleteById`). Deleting a `User` revokes all their refresh tokens. Deactivating (`isActive: false`) or deleting a user also immediately zeroes out their effective permissions on their next request, even on an unexpired access token — `AuthorizationService` checks `isActive`, not just JWT validity.
...
- **Phase 6** added `POST /api/v1/simulator/check` (gated on `simulator.run`): given either a real user or a hypothetical set of role ids, plus a permission name, returns whether it'd be granted, which role(s) grant it, and the full resolved permission set — a pure read, nothing is ever mutated or actually granted. A deactivated user always resolves to `allowed: false` (mirrors `resolveAccess`'s real behavior), but the response still shows what *would* grant it, for diagnostic purposes. See `backend/Phase-06.md`.
- `User`, `Permission`, `Role`, `AuditLog`, and `Policy` schemas all set `toJSON: toJSONOptions` (`src/models/schemaOptions.ts`) so API responses expose `id` instead of Mongo's `_id`/`__v` — Mongoose does **not** do this by default (its `id` virtual isn't included in `toJSON()` output unless configured). If you add a new model whose raw documents get serialized directly to a client, apply the same option or you'll silently leak `_id` instead of `id`.
