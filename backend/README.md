...
**Bootstrap:** the very first user ever registered (in an empty database) automatically gets an `admin` role bundling a baseline set of 12 permissions (all CRUD actions on permissions/roles/users, plus `auditlogs.read` — see `src/constants/systemPermissions.ts`), so a fresh deployment isn't locked out of managing its own RBAC data. Every subsequent registration gets zero roles/permissions and must be granted access by an existing admin via `PATCH /users/:id`.
...
`npm test` runs 38 tests across five suites (`auth`, `permissions`, `roles`, `users`, `auditLog`) via Jest + Supertest against an in-memory MongoDB (`mongodb-memory-server`) — no external database needed. Phase 3 adds one 403-on-missing-permission test per resource, using `tests/helpers/authenticatedUser.ts`'s `createNonAdminUser` (registers a throwaway first user to consume the admin-bootstrap slot, then returns a second, genuinely permission-less user).

See [Phase-01.md](Phase-01.md), [Phase-02.md](Phase-02.md), [Phase-03.md](Phase-03.md), [Phase-04.md](Phase-04.md), and [Phase-05.md](Phase-05.md) for end-of-phase summaries.
